...
class BlockDeviceWrapper(update_coordinator.DataUpdateCoordinator):
    ...
    def _mc_send_event(self, channel, delete, momentary_button, click_count, click_events):
        if momentary_button:
        ...
        for block in self.device.blocks:
            ...
            if block.inputEventCnt is not None:
                if self._mc_click_count.get(channel) is None:
                    self._mc_click_count[channel] = 0
                    self._mc_last_events[channel] = ""
                if last_event_count is not None and self._mc_last_state.get(channel) is not None:
                    if block.inputEventCnt != last_event_count:
                        ...
                        if mc_curr_event == "L":
                            if mc_momentary_button:
                                if block.input != self._mc_last_state.get(channel):
                                    self._mc_click_count[channel] += 1
                                self._mc_send_event(channel, False, mc_momentary_button, 0, self._mc_last_events[channel] + "LSTART")
                            else:        
                        ...
                        if not mc_momentary_button or block.input == 0:
                            self._mc_click_timer[channel] = Timer(MC_MULTICLICK_DELAY, self._mc_send_event, args=(channel, True, mc_momentary_button, self._mc_click_count[channel], self._mc_last_events[channel]))
                    ...
                    if block.inputEventCnt == last_event_count and block.input != self._mc_last_state.get(channel):
                        ...
                        if mc_momentary_button and block.input == 0 and len(self._mc_last_events[channel]) > 0:
                            if self._mc_last_events[channel][-1] == "L":
                                self._mc_send_event(channel, False, mc_momentary_button, 0, self._mc_last_events[channel] + "STOP")
